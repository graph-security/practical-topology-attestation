...
import uk.ac.ncl.cascade.daa.join.DAACredential;
...
public class Topographia {
	...
	private static StringBuilder perfData;
	...
	public static void main(String[] argv) throws IOException, ClassNotFoundException, ProofStoreException, NoSuchAlgorithmException, ImportException, EncodingException, InterruptedException {
		...
		// adding clients ip addressses
		clients.add("192.168.2.10");
		clients.add("192.168.2.12");
		clients.add("192.168.2.9");
		clients.add("192.168.2.11");
		perfData = new StringBuilder();
		try {
			parser.parse(argv);
		...
		try {
			...
			if (keygenMode != null && keygenMode.booleanValue()) {
				...
				if (bindingCredMode != null && bindingCredMode) {
                    ...
                    // execute proof of possession of a device credential
//					pseudonym = topographia.daaSign();
//					System.out.println("Pseudonym from daaCredential: " + pseudonym);
				...
				perfData.append("verify_bc,issuing_grs \n");
	...
	private String daaSign() throws IOException {
        // execute DAA Sign protocol for proving possession of a device credential
		DAASignLib daaSignLib = new DAASignLib(); 
		daaSignLib.loadLib();
		daaSignLib.executeDAA();
		DAACredential daaCredential = new DAACredential(); 
		daaCredential.readCredentialFile();
		return daaCredential.getCapitalNi();
	}
	...
	void signbc(ExtendedKeyPair ekp, String nym, String hostAddress, int portNumber) throws IOException, ClassNotFoundException, InterruptedException {
		...
		SignerOrchestratorBC signer = new SignerOrchestratorBC(ekp, messageGateway);
		...
		System.out.println(" Sign: requesting a proof of possession of device credential.");
		try {
			signer.daaSignVerifier();
			nym = signer.getNG();
			...
			handleException(e, "The proof of possession of device credential failed", TopographiaErrorCodes.EX_VERIFY);
		}
...
//		hostAddress = "192.168.2.1";
		...
		long verifyvc_perf = 0;
		long issue_grs_perf = 0;
		verifyvc_perf = System.nanoTime();
...
//			System.out.println("portNumber: " + portNumber);
		...
		verifyvc_perf = System.nanoTime() - verifyvc_perf;
...
//		hostAddress = "192.168.2.1";
		...
		System.out.print("  Sign: Initializing the Signer role...");
		issue_grs_perf = System.nanoTime();
		try {
			signer.init();
		} catch (IOException e) {
			handleException(e, "The TOPOGRAPHIA Signer could not establish a connection to the Recipient in Round 0.",
...
//		System.out.print("  Sign - Round 2: Waiting for the Recipient's commitment...");
		...
		issue_grs_perf = System.nanoTime() - issue_grs_perf;
		...
		System.out.println("  Sign: Completed");
		perfData.append(TimeUnit.MILLISECONDS.convert(verifyvc_perf, TimeUnit.NANOSECONDS)+",");
		perfData.append(TimeUnit.MILLISECONDS.convert(issue_grs_perf, TimeUnit.NANOSECONDS));
		saveCSV(perfData, "perf-issue.csv");
		System.out.println("results: " + perfData);
	}
	...
	void receivebc(String graphFilename, String sigmaFilename, int i, String hostAddress, int portNumber) throws InterruptedException, IOException {
		...
		System.out.print("  Receive: proof of possession of device credential...");
		try {
			recipient.daaSignProver();
		} catch (IOException e) {
			handleException(e, "The TOPOGRAPHIA Recipient could not finish proof of possession of device credential.",
					TopographiaErrorCodes.EX_PROTOCOL);
		}
		System.out.println("   [done]");
		System.out.println();
	...
	void provevc(String graphFilename, String bCredFilename, String hostAddress, Integer portNumber) throws ProofStoreException, NoSuchAlgorithmException, InterruptedException, IOException {
		System.out.println("  Prove: Acts as host prover for binding credential.");
		System.out.println("hostAddress: " + hostAddress);
		System.out.println("portNumber: " + portNumber);
		perfData.append("provebc \n");
		long provebc = 0;
		IMessageGateway messageGateway = new MessageGatewayProxy(SERVER, hostAddress, portNumber);
		...
		provebc = System.nanoTime() - provebc;
		...
		perfData.append(TimeUnit.MILLISECONDS.convert(provebc, TimeUnit.NANOSECONDS) + ",");
		saveCSV(perfData, "prove-binding-credential.csv");
		System.out.println("results prove binding credentials: " + perfData);
	}
	...
	void proveBindings(String graphFilename, String sigmaFilename, String hostAddress, Integer portNumber) throws IOException, NoSuchAlgorithmException, ProofStoreException, InterruptedException, EncodingException {
		...
		perfData.append("verify_bc,pop_grs \n");
		long verify_bc = 0;
		long pop_grs = 0;

		List<String> pslist = persistenceUtil.readFileLines(DEF_PSEUDONYMS);
		verify_bc = System.nanoTime();
		for (int i = 0; i < pslist.size(); i++) {
//			System.out.println("Iteration: " + i);
//			System.out.println("hostaddress: " + hostAddress);
//			System.out.println("portNumber: " + portNumber);
		...
		verify_bc = System.nanoTime() - verify_bc;
...
//			System.out.println("portNumber: " + port);
		...
		pop_grs = System.nanoTime() - pop_grs;

		System.out.println("   [done]");
		System.out.println("  Prove: Signature proof of knowledge sent to Verifier.");

		System.out.println("  Prove: Completed");
		perfData.append(TimeUnit.MILLISECONDS.convert(verify_bc, TimeUnit.NANOSECONDS)+",");
		perfData.append(TimeUnit.MILLISECONDS.convert(pop_grs, TimeUnit.NANOSECONDS));
		saveCSV(perfData, "pop-grs.csv");
		System.out.println("results: " + perfData);
	}
...
//		Thread.sleep(1000);
	...
	private void saveCSV(StringBuilder perfData, String filename) throws IOException {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename, true))) {
			writer.newLine();
			writer.write(perfData.toString());
		} catch (IOException e) {
