...
Tcsip_TPM_DAA_Join_encapsulate(TCS_CONTEXT_HANDLE tcsContext, // in
    ...
    LogDebug("Tcsip_TPM_DAA_Join_encapsulate(TCS_CONTEXT=%X,TSS_HDAA=%X,stage=%d,\
    ...
    LogDebug("Tcsip_TPM_DAA_Join_encapsulate(stage=%d,outputSize=%u outputData=%s ownerAuth=%X) result=%d",
                                   ...
                                   TSS_DAA_JOIN_SESSION *join_session)
{
    ...
    // i represents l_k in the TSS specification
    LogDebug("start joininit stage 0");
    ...
    for(i =0; i< issuer_authentication_PKLengh; i++) {
        bi_set_as_BIGNUM(issuer_authentication_PK_i, (BIGNUM *) RSA_get0_n(issuer_authentication_PK[i]));
        bi_2_byte_array( buffer_modulus,
                         modulus_length,
                         issuer_authentication_PK_i);
        dump_byte_array(256, buffer_modulus);
    ...
    join_session->issuerPk = *issuer_pk;
    if( result == TSS_SUCCESS) {
    ...
    // get the endorsement Key (public part)o
    UINT32 endLength;
    BYTE * endCred;
    ...
    LogDebug("endorsement key=%s", dump_byte_array( endLength, endCred));

    identity_proof->endorsementLength = outputSize;
    identity_proof->endorsementCredential = outputData;
close:
    FREE_BI( issuer_authentication_PK_i);
    LogDebug("DAA_Join close -> result = %d", result);
...
//		(int)hDAA, (int)hTPM, daaCounter, (int)issuerPk);
    ...
    LogDebug("-> TSPI_TPM_DAA_joinInit hDAA=%x hTPM=%x daaCounter=%x issuerPk",
             (int)hDAA, (int)hTPM, daaCounter);
...
anonymity of signatures will hod.
...
Tspi_DAA_IssueSetup(TSS_HDAA       hDAA,			// in
		    ...
		    UINT32         numberIssuerAttributes,	// in
                    TSS_DAA_KEY_PAIR**                  keyPair,
                    TSS_DAA_PK_PROOF**       publicKeyProof)		// out (TSS_DAA_PK_PROOF)
{
	TSS_RESULT result;
	KEY_PAIR_WITH_PROOF_internal *key_proof;
	TSS_DAA_KEY_PAIR *tss_daa_key_pair;
	TSS_HCONTEXT hContext;
    TSS_DAA_PK_PROOF *pkProof;
    TSS_DAA_SK *skey;
    ...
    pkProof = i_2_e_TSS_DAA_PK_PROOF(key_proof->proof, &tss_alloc, hContext);


    LogDebug("convert to public key proof");
    *publicKeyProof = pkProof;

	tss_daa_key_pair = (TSS_DAA_KEY_PAIR *)tss_alloc( sizeof(TSS_DAA_KEY_PAIR), hContext);
    LogDebug("allocate memory for tss daa keypair");
    ...
    LogDebug("Allocate secret key...");

    TSS_DAA_PRIVATE_KEY *seckey;
    seckey = i_2_e_TSS_DAA_PRIVATE_KEY( key_proof->private_key,
                        &tss_alloc,
                            hContext);
    tss_daa_key_pair->secretKey.productPQprime = seckey->productPQprime;
    tss_daa_key_pair->secretKey.productPQprimeLength = seckey->productPQprimeLength;
    tss_daa_key_pair->versionInfo = seckey->versionInfo;

    LogDebug("Allocate public key...");
	tss_daa_key_pair->publicKey = *i_2_e_TSS_DAA_PK( key_proof->pk,
							&tss_alloc,
							hContext);
	*keyPair = tss_daa_key_pair;

    LogDebug("Allocate key pair...");
close:
	bi_flush_memory();
			    ...
			    BYTE*               issuerBaseName)       // in
{
	...
	// prepare out parameters
    TSS_DAA_PK_PROOF **publicKeyProof = NULL;
    *publicKeyProof = i_2_e_TSS_DAA_PK_PROOF(key_proof->proof, &tss_alloc, tspContext);
    ...
    // TODO fix retrieving private key
    TSS_DAA_PRIVATE_KEY *sKey = i_2_e_TSS_DAA_PRIVATE_KEY(key_proof->private_key, &tss_alloc, &tspContext);

	tss_daa_key_pair->publicKey = *i_2_e_TSS_DAA_PK( key_proof->pk,
							&tss_alloc,
							tspContext);
    TSS_HKEY *keyPair = NULL;
           ...
           BYTE** joinSession)                   // out
{
	...
	int before = mallinfo().uordblks;
#endif
    TSS_DAA_IDENTITY_PROOF *idProof = &identityProof;
//    BYTE **authChallenge = authenticationChallenge;
	...
	result = Tspi_DAA_IssueInit_internal(
		...
		issuerAuthPK,		// in
		issuerKeyPair,		// in
        (TSS_DAA_IDENTITY_PROOF *) idProof,		// in
...
Tspi_DAA_IssueCredential(TSS_HDAA                      hDAA,			// in
			 ...
			 struct tdTSS_DAA_CREDENTIAL_REQUEST   credentialRequest,	// in
			 struct tdTSS_DAA_JOIN_ISSUER_SESSION  joinSession,		// in
...
Tspi_DAA_VerifySignature(
			 ...
			 TSS_DAA_PK      hPubKeyIssuer,	// in (TSS_DAA_PK)
		     ...
		     TSS_HDAA_DATA* arPrivateKey)	// out (TSS_DAA_AR_SK)
{
	...
	int before = mallinfo().uordblks;
#endif
// TODO add public and private key pair in keypair for now send the arPublickey
//	result = Tspi_DAA_RevokeSetup_internal(
//		hDAA,			// in
////TODO: remove cast when the above interface is changed
//		daaPublicKey,	// in
////		keyPair		// out -fixes implementation error for missing the keypair
//        arPublicKey // out
//	);
//	bi_flush_memory();
#ifdef TSS_DEBUG
		   ...
		   TSS_DAA_PSEUDONYM_PLAIN**    pseudonym)		// out
{
	...
	result =0;
    // TODO fix undefined reference for Tspi_DAA_ARDecrypt_internal
//    Tspi_DAA_ARDecrypt_internal(
//		hDAA,			// in
...
(
	...
	TSS_DAA_CREDENTIAL **hDaaCredential	// out
) {
	TCS_CONTEXT_HANDLE tcsContext;
	TPM_AUTH ownerAuth;
    ...
    LogDebug("enter JoinStoreCredential_internal ");
	...
	daa_pk_extern = &joinSession.issuerPk;
...
//	}
    ...
    LogDebug("compute hat A vPrimePrime: %s", bi_2_hex_char(v_prime_prime));
    ...
    LogDebug("compute hat A vPrimePrime: %s", bi_2_hex_char(v_prime_prime));
    LogDebug("compute hat A capitalS: %s", bi_2_hex_char(pk_intern->capitalS));
    LogDebug("compute hat A capitalU: %s", bi_2_hex_char(capital_U));
	...
	LogDebug("compute hat A: encode attributes");
...
// TODO enable attributes
	...
	bi_mod( fraction_A, fraction_A, n);
    LogDebug("compute hat A: intermediate fraction_A: %s", bi_2_hex_char(fraction_A));
    ...
    LogDebug("compute hat A: s_e [%ld]=%s", bi_nbin_size(s_e), bi_2_hex_char( s_e));

	bi_mod_exp( capital_Atilde, fraction_A, s_e, n);
    ...
    bi_ptr r_e;
    bi_ptr temp_A = bi_new_ptr();
    bi_mod_exp(temp_A, capital_A, c_prime, pk_intern->modulus);
    LogDebug("issue temp_A: %s", bi_2_hex_char(temp_A));

    LogDebug("compute hat A: product attributes [%ld]=%s", bi_nbin_size(product_attributes), bi_2_hex_char( product_attributes));
    LogDebug("compute hat A: capital_A  [%ld]=%s", bi_nbin_size(capital_A), bi_2_hex_char( capital_A));
    LogDebug("compute hat A: c_prime  [%ld]=%s", bi_nbin_size(c_prime), bi_2_hex_char( c_prime));
    LogDebug("compute hat A: modulus n  [%ld]=%s", bi_nbin_size(n), bi_2_hex_char(n));
    bi_ptr tmp3 = bi_new_ptr();
    bi_mod_exp( tmp3, capital_A, c_prime, n);
	bi_mul( capital_Atilde, capital_Atilde, tmp3);
	bi_mod( capital_Atilde, capital_Atilde, n);

    LogDebug("compute hat A: capital_Atilde hat  [%ld]=%s", bi_nbin_size(capital_Atilde), bi_2_hex_char( capital_Atilde));
	...
	if( result != TSS_SUCCESS) goto close;
    LogDebug("credIssuer.cPrime %s", dump_byte_array(20, credIssuer.cPrime));
    LogDebug("c_byte %s", dump_byte_array(20, c_byte));
    ...
    LogDebug("compute hat A: s_e %s", bi_2_hex_char(s_e));

	bi_mod_exp( product, capital_A, e, n);
    bi_ptr fraction_B= bi_new_ptr();
    bi_mod_exp(fraction_B , pk_intern->capitalS, v_prime_prime, n);
    bi_mul( fraction_B, fraction_B, capital_U);
    bi_mod( fraction_B, fraction_B, n);
	bi_mul( product, product, fraction_B);
	bi_mod( product, product, n);
	if( bi_equals( pk_intern->capitalZ, product) == 0) {
		LogError("Verification of A failed - Step 1.c.ii");
        LogError("\tfraction_A=%s", bi_2_hex_char( fraction_B));
        LogError("\te=%s", bi_2_hex_char(e));
        LogError("\tmodulus n=%s", bi_2_hex_char(n));
	...
	result = Tcsip_TPM_DAA_Join_encapsulate(tcsContext, hDAA,
                                            ...
                                            0, NULL,
                                            &ownerAuth, &v0Length,  &outputData);
	if( result != TSS_SUCCESS) goto close;
   v0 = outputData;
	LogDebug("Done Join 22: return v0 %s", dump_byte_array(v0Length, v0));
	free( c_byte);
	...
	result = Tcsip_TPM_DAA_Join_encapsulate(tcsContext, hDAA,
                                            ...
                                            0, NULL,
                                            0, NULL,
                                            &ownerAuth, &tpm_specificLength, (BYTE **) &tpm_specific);
	if( result != TSS_SUCCESS) goto close;

    LogDebug("end of daa join stage 24");
    ...
    LogDebug("allocate capital A for credential ");

//	bi_2_nbin1((int *) daaCredential->capitalALength, daaCredential->capitalA, capital_A);
    ...
    LogDebug("allocate exponent for credential ");
    ...
    LogDebug("allocate vBar0 for credential");
...
//	}
    // TODO fix attributes for credential
//	for( i=0; i < credIssuer.attributesIssuerLength; i++) {
...
Tspi_TPM_DAA_JoinCreateDaaPubKey_internal(
    ...
    LogDebug("enter JoinCreateDaaPubKey_internal");



    TSS_DAA_PK* pk_extern = &joinSession->issuerPk;
...
//                capitalU);
    LogDebug("Update credential request ");
    ...
    // TODO fix capitalULength = 256
    credentialRequest->capitalULength = 256;
    UINT32  capUlength = credentialRequest->capitalULength;
    bi_2_nbin1(&capUlength,
               credentialRequest->capitalU,
               capitalU);
    ...
    LogDebug("finish credential request");
...
Tspi_TPM_DAA_JoinStoreCredential(TSS_HDAA              hDAA,		// in
				 ...
				 TSS_DAA_CRED_ISSUER*  credIssuer,	// in
				 TSS_DAA_JOIN_SESSION* joinSession,	// in
                 TSS_DAA_CREDENTIAL**        phDaaCredential)	// out (TSS_DAA_CREDENTIAL)
{
	TSS_RESULT result;
#ifdef TSS_DEBUG
	int before = mallinfo().uordblks;
...
(
	TSS_HDAA	hDAA,	// in
	TSS_HTPM	hTPM,	// in
	TSS_DAA_CREDENTIAL  hDaaCredential,	// in
...
//	}
    ...
    LogDebug("capital_Nv %s", bi_2_hex_char(capital_Nv));
    ...
    LogDebug("capital_N_tilde_v %s", bi_2_hex_char( capital_N_tilde_v));
    ...
    LogDebug("capital_A %s", bi_2_hex_char(capital_A));
    ...
    LogDebug("capital_T %s", bi_2_hex_char(capital_T));

	bi_mul( capital_T_tilde, t_tilde_T, tmp1);

    LogDebug("t_tilde_T %s ", bi_2_hex_char(t_tilde_T));

	bi_mod( capital_T_tilde, capital_T_tilde, n);

    LogDebug("pk_intern->capitalS %s ", bi_2_hex_char(pk_intern->capitalS));
...
//    bi_set( product_R, bi_1);
//    LogDebug("product_R: %s", bi_2_hex_char(product_R));
//
	bi_mul( capital_T_tilde, capital_T_tilde, product_R);
	bi_mod( capital_T_tilde, capital_T_tilde, n);
    LogDebug("capital_TTilde: %s", bi_2_hex_char(capital_T_tilde));
    ...
    UINT32 capNvLength = pseudonym_plain->capitalNvLength;
	bi_2_nbin1( &capNvLength,
				pseudonym_plain->capitalNv,
				capital_Nv);
    pseudonym_plain->capitalNvLength = capNvLength;
	...
	pseudonym_plain_tilde->capitalNv = calloc_tspi( tcsContext, bi_nbin_size( capital_N_tilde_v));
    ...
    UINT32 capNvTildeLength = pseudonym_plain_tilde->capitalNvLength;
	bi_2_nbin1( &capNvTildeLength,
				pseudonym_plain_tilde->capitalNv,
				capital_N_tilde_v);
    pseudonym_plain_tilde->capitalNvLength = capNvTildeLength;
    LogDebug("capital_T_tilde: %s", bi_2_hex_char(capital_T_tilde));
    ...
    LogDebug("signature->sF0: %s", bi_2_hex_char(sF0));
	free( outputData);
	result = Tcsip_TPM_DAA_Sign(
		tcsContext, hDAA,
		12,
    ...
    LogDebug("signature->sF1: %s", bi_2_hex_char(sF1));
	free( outputData);
	result = Tcsip_TPM_DAA_Sign(
		tcsContext, hDAA,
		13,
    ...
    LogDebug("signature->sV: %s", bi_2_hex_char(s_V));
	// attributes extension
...
//			bi_free_ptr( tmp2);
    ...
    //&daaSignature->zetaLength,
    int zetalength = daaSignature->zetaLength;
	bi_2_nbin1(&zetalength, daaSignature->zeta, zeta);
    daaSignature->zetaLength = zetalength;
...
//	daaSignature->signedPseudonym = *signature_pseudonym;
    daaSignature->signedPseudonym.payload = pseudonym_plain->capitalNv;
    daaSignature->signedPseudonym.payloadLength = pseudonym_plain->capitalNvLength;
...
Tspi_TPM_DAA_Sign(
        TSS_HDAA                 hDAA,			// in
		  TSS_HTPM                 hTPM,			// in
          TSS_DAA_CREDENTIAL    hDaaCredential,		// in (TSS_DAA_CREDENTIAL)
		  TSS_DAA_SELECTED_ATTRIB* revealAttributes,		// in
		  UINT32                   verifierBaseNameLength,	// in
		  BYTE*                    verifierBaseName,		// in
		  UINT32                   verifierNonceLength,		// in
...
//
	...
	bi_flush_memory();
#ifdef TSS_DEBUG
	LogDebug("TSPI_TPM_DAA_Sign ALLOC DELTA:%d", mallinfo().uordblks-before);
#endif
	LogDebug("<- TSPI_TPM_DAA_Sign result=%d", result);
	return result;
}
