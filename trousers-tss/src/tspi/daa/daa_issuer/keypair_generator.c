		  ...
		  KEY_PAIR_WITH_PROOF_internal** key_pair_with_proof)
{
	...
	*key_pair_with_proof = NULL;
    ...
    LogDebug("modulus n: %s", bi_2_hex_char(n));
    ...
    LogDebug("pPrime: %s", bi_2_hex_char(pPrime));
    LogDebug("qPrime: %s", bi_2_hex_char(qPrime));
	// STEP 2
	...
	TSS_DAA_PK_PROOF_internal *correctness_proof = generate_proof(product_PQprime, public_key, xz, x0, x1, x);
    LogDebug("correctness_proof ....");
	...
	*key_pair_with_proof = (KEY_PAIR_WITH_PROOF_internal *) malloc(sizeof(KEY_PAIR_WITH_PROOF_internal));
    LogDebug("key pair with proof ....");
    ...
    LogDebug("Create daa private key...");
	// all fields are linked to the struct with direct reference
	(*key_pair_with_proof)->private_key = create_TSS_DAA_PRIVATE_KEY(pPrime, qPrime);

    LogDebug("Finish creation of keypair with proof...");
close:
	if (result != TSS_SUCCESS) {
		LogDebug("Clear numbers...");
	...
	*/
	...
	bi_free(bi_tmp1);
	bi_free(bi_tmp2);
    LogDebug("finish freeing temp values..");
	FREE_BI(p);
	FREE_BI(q);
	FREE_BI(product_PQprime);
    LogDebug("finish freeing p, q , product pq values..");
	FREE_BI(pair[ELEMENT]);
	FREE_BI(pair[EXPONENT]);
    LogDebug("finish freeing pairs..");
	FREE_BI(xz);
	FREE_BI(x0);
	FREE_BI(x1);
    LogDebug("finish freeing x..");
	// bi_array_ptr x = NULL;
	// bi_array_ptr capital_r = NULL;
	// bi_array_ptr capitalRReceiver = NULL;
	// bi_array_ptr capitalRIssuer = NULL;
