...
TSS_RESULT verify_authentificity(TSS_DAA_CREDENTIAL_REQUEST *credentialRequest,
	...
	char *buffer;
    LogDebug("enter verify authenticity");

	modulus_N0 = bi_new_ptr();
    RSA *iapk = (RSA *) joinSession->issuerAuthPK;
    BIGNUM *bn_modulus = (BIGNUM *) RSA_get0_n(iapk);
...
compute_join_challenge_issuer( TSS_DAA_PK_internal *pk_intern,
    ...
    LogDebug("compute challenge: capitalA   [%ld]=%s", bi_nbin_size(capitalA), bi_2_hex_char( capitalA));
    LogDebug("compute challenge: capital_Atilde   [%ld]=%s", bi_nbin_size(capital_Atilde), bi_2_hex_char( capital_Atilde));
    LogDebug("compute challenge: v_prime_prime   [%ld]=%s", bi_nbin_size(v_prime_prime), bi_2_hex_char( v_prime_prime));
    LogDebug("compute challenge: nonceReceiver %s", dump_byte_array(nonceReceiverLength, nonceReceiver));
...
compute_credential_proof( TSS_DAA_PK_internal *pk_intern,
    ...
    LogDebug("compute proof A: capital_A  [%ld]=%s", bi_nbin_size(capital_A), bi_2_hex_char( capital_A));
    LogDebug("compute proof productPQprime:   [%ld]=%s", bi_nbin_size(productPQprime), bi_2_hex_char( productPQprime));

	bi_urandom( random_E, bi_length( productPQprime) + DAA_PARAM_SAFETY_MARGIN * 8);
	bi_mod( random_E, random_E, productPQprime);
//    bi_mod( random_E, random_E, pk_intern->modulus);
	bi_inc( random_E);

    LogDebug("inside fraction %s", bi_2_hex_char(fraction_A));

	bi_mod_exp( capital_Atilde, fraction_A, random_E, pk_intern->modulus);

    LogDebug("compute tilde A: random_E  [%ld]=%s", bi_nbin_size(random_E), bi_2_hex_char(random_E));
    LogDebug("compute tilde A: fraction_A  [%ld]=%s", bi_nbin_size(fraction_A), bi_2_hex_char(fraction_A));
    LogDebug("compute tilde A: capital_Atilde [%ld]=%s", bi_nbin_size(capital_Atilde), bi_2_hex_char( capital_Atilde));
    ...
    LogDebug("after compute_join_challenge_issuer c_prime_bytes %s", dump_byte_array(20, c_prime_bytes));
    LogDebug("after compute_join_challenge_issuer eInverse %s", bi_2_hex_char(eInverse));
    LogDebug("after compute_join_challenge_issuer producPQprime %s", bi_2_hex_char(productPQprime));
    LogDebug("after compute_join_challenge_issuer random_E %s", bi_2_hex_char(random_E));


	*c_prime = bi_set_as_nbin( c_primeLength, c_prime_bytes); // allocation

    LogDebug("after compute_join_challenge_issuer c_prime %s", bi_2_hex_char(*c_prime));
	*s_e = bi_new_ptr();
	bi_mul( *s_e, *c_prime, eInverse);
	bi_mod( *s_e, *s_e, productPQprime);
	bi_sub( *s_e, random_E, *s_e);
	bi_mod( *s_e, *s_e, productPQprime);
    LogDebug("after compute_join_challenge_issuer s_e %s", bi_2_hex_char(*s_e));
...
(
    ...
    LogDebug("enter issue credential internal");
	tmp1 = bi_new_ptr();
	tmp2 = bi_new_ptr();
...
//	daa_pk_extern = (TSS_DAA_PK *)(((TSS_DAA_KEY_PAIR *)joinSession.issuerKeyPair)->publicKey);
    TSS_DAA_KEY_PAIR *iskeypair =  &joinSession.issuerKeyPair;
    daa_pk_extern = (TSS_DAA_PK ) iskeypair->publicKey;
...
// TODO enable attributes
    ...
    LogDebug("compute tilde A vPrimePrime: %s", bi_2_hex_char(v_prime_prime));
    LogDebug("compute tilde A capitalS: %s", bi_2_hex_char(pk_intern->capitalS));
    LogDebug("compute tilde A capitalU: %s", bi_2_hex_char(capitalU));
    ...
    // TODO enable attributes
    bi_set( product_attr_issuer, bi_1);
    LogDebug("product_attr_issuer %s", bi_2_hex_char(product_attr_issuer));
    ...
    LogDebug("intermediate fraction_A: %s", bi_2_hex_char(fraction_A));

    TSS_DAA_KEY_PAIR *is = &joinSession.issuerKeyPair;
    TSS_DAA_SK secret_key= is->secretKey;
	bi_free_ptr( tmp2);

	tmp2 = bi_set_as_nbin( secret_key.productPQprimeLength,
		secret_key.productPQprime); // allocation
	if( tmp2 == NULL) {
    ...
    LogDebug("issuer product attributes [%ld]=%s", bi_nbin_size(product_attr_issuer ), bi_2_hex_char( product_attr_issuer));
	...
	compute_credential_proof( pk_intern,
				...
				&s_e);	// out: allocation
    LogDebug("c_prime out [%ld]=%s", bi_nbin_size(c_prime), bi_2_hex_char( c_prime));
    LogDebug("s_e out [%ld]=%s", bi_nbin_size(s_e), bi_2_hex_char( s_e));
    bi_ptr temp_A = bi_new_ptr();
    bi_mod_exp(temp_A, capitalA, c_prime, pk_intern->modulus);
    LogDebug("issue temp_A: %s", bi_2_hex_char(temp_A));
	// populate credIssuer (TSS_DAA_CRED_ISSUER *)
    LogDebug("populate credIssuer");

    BYTE *capA = calloc_tspi(tcsContext, bi_nbin_size(capitalA));
	if( capA == NULL) {
    ...
    // TODO fix capital length
    credIssuer->capitalALength = 256;
	bi_2_nbin1( &credIssuer->capitalALength, capA, capitalA);
    credIssuer->capitalA = capA;
...
//	}
    ...
    LogDebug("cprime: %s", c_prime);
    LogDebug("cprime bin: %s", dump_byte_array(credIssuer->cPrimeLength, credIssuer->cPrime));
    ...
    LogDebug("issue credential tilde A: s_e [%ld]=%s", bi_nbin_size(s_e), bi_2_hex_char( s_e));
    LogDebug("finish issue credential internal");
close:
	EVP_MD_CTX_destroy(mdctx);
