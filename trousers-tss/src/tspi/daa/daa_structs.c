...
//    printf("store bi:\n");
    bufferbi =  (BYTE *) daa_alloc(bi_length(i), object);
    bi_2_nbin1(&size, bufferbi, i);
    *length = size;
    char *byt = dump_byte_array(size, bufferbi);
//    printf("bytearray: %s \n", byt);
//	LogDebug( "[store_bi] host_length:%d network_length:%d[address:%s]\n", size, (int)*length, byt);
    return bufferbi;
}

bi_ptr get_bi( const unsigned long n_length, const BYTE *buffer_bi) {
	unsigned long length;

//	length = n_length;
    LogDebug("get_bi: length: %lu ", n_length);
//	printf( "[get_bi] %d [address:%d -> |%2x|%2x| ]\n",
//		(int)length,
//		(int)buffer,
//		(int)(buffer[0] &0xFF),
//		(int)(buffer[1]&0xFF));
	return bi_set_as_nbin( n_length, buffer_bi);
}
...
DAA_PRIVATE_KEY_internal *e_2_i_TSS_DAA_PRIVATE_KEY(TSS_DAA_PRIVATE_KEY *private_key) {
	...
	private_key_internal =
		(DAA_PRIVATE_KEY_internal *)malloc( sizeof(DAA_PRIVATE_KEY_internal));
    if (private_key_internal == NULL) {
		LogError("Can not allocate the DAA_PRIVATE_KEY_internal structure");
		return NULL;
	}
			  ...
			  TSS_HOBJECT param_alloc)
{
	...
	result->p_prime = store_bi( &result->p_primeLength,
              result->p_prime,
			private_key_internal->p_prime,
			daa_alloc,
			param_alloc);
    result->q_prime = 	store_bi( &result->q_primeLength,
			result->q_prime,
			private_key_internal->q_prime,
			daa_alloc,
			param_alloc);
    result->productPQprime = store_bi( &result->productPQprimeLength,
				       ...
				       TSS_HOBJECT param_alloc)
{
	...
	init_tss_version(result);
	pkey = i_2_e_TSS_DAA_PRIVATE_KEY(key_pair_with_proof->private_key,
							daa_alloc, param_alloc);

    sk = (TSS_DAA_SK *)daa_alloc(sizeof(TSS_DAA_SK), param_alloc);
    init_tss_version(sk);
    sk->productPQprime = pkey->productPQprime;
    sk->productPQprimeLength = pkey->productPQprimeLength;

    result->secretKey = *sk;
	result->publicKey = *(TSS_DAA_PK *) i_2_e_TSS_DAA_PK( key_pair_with_proof->pk, daa_alloc, param_alloc);
		 ...
		 TSS_HOBJECT param_alloc)
{
    ...
    pk->modulus = store_bi( &pk->modulusLength,
		pk->modulus,
		pk_internal->modulus,
		daa_alloc,
		param_alloc);
    pk->capitalS = store_bi( &pk->capitalSLength,
                             pk->capitalS,
                             pk_internal->capitalS,
                             daa_alloc,
                             param_alloc);

    pk->capitalZ = store_bi( &pk->capitalZLength,
                             pk->capitalZ,
                             pk_internal->capitalZ,
                             daa_alloc,
                             param_alloc);
    pk->capitalR0 = store_bi( &pk->capitalR0Length,
                             pk->capitalR0,
                             pk_internal->capitalR0,
                             daa_alloc,
                             param_alloc);

    pk->capitalR1 = store_bi( &pk->capitalR1Length,
                              pk->capitalR1,
                              pk_internal->capitalR1,
                              daa_alloc,
                              param_alloc);
    pk->gamma = store_bi( &pk->gammaLength,
                              pk->gamma,
                              pk_internal->gamma,
                              daa_alloc,
                              param_alloc);
    pk->capitalGamma = store_bi( &pk->capitalGammaLength,
                          pk->capitalGamma,
                          pk_internal->capitalGamma,
                          daa_alloc,
                          param_alloc);
    pk->rho= store_bi( &pk->rhoLength,
                                 pk->rho,
                                 pk_internal->rho,
                                 daa_alloc,
                                 param_alloc);
//	STORE_DAA_PK_BI( pk->modulus, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->capitalS, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->capitalZ, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->capitalR0, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->capitalR1, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->gamma, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->capitalGamma, daa_alloc, param_alloc);
//	STORE_DAA_PK_BI( pk->rho, daa_alloc, param_alloc);
...
#ifdef DAA_DEBUG
	printf("\n create_DAA_PK_PROOF_internal\n");
#endif

	pk_proof = (TSS_DAA_PK_PROOF_internal *)malloc( sizeof(TSS_DAA_PK_PROOF_internal));
    if (pk_proof == NULL) {
        LogError("TSS_DAA_PK_PROOF_internal malloc of %d bytes failed", sizeof(TSS_DAA_PK_PROOF_internal));
        return NULL;
    }
		       ...
		       TSS_HOBJECT param_alloc)
{
	...
	length_response3 = bi_nbin_size(pk_internal_proof->response[0]->array[0]);

    LogDebug("length response: %d", pk_proof->responseLength);
    LogDebug("length_response 2: %d", length_response2);
    LogDebug("length_response 3: %d", length_response3);

	if( length_response3 & 1) length_response3++; // length_response3 should be paire

    LogDebug("length_response 3b: %d", length_response3);
...
//        char *out = dump_byte_array(length_response2, pk_proof->response[i]);
//        LogDebug("i=%d byte dump  %s ",i, pk_proof->response[i]);
//        bi_ptr bi_ret = bi_set_as_nbin(length_response2, pk_proof->response[i]);
//        LogDebug("i=%d conversion responses: %s ", i,  bi_2_hex_char(bi_ret));
...
//        bi_array_ptr ret_bi = pk_proof_internal->response[i];
//        printf("recreate bi %s \n", bi_2_hex_char(*ret_bi->array));
	}
