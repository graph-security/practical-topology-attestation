...
//			LogDebug("signature=%s\n", dump_byte_array(256, signature));
            ...
            bi_set_as_hex( bi, BN_bn2hex((BIGNUM *) RSA_get0_n(rsa)));
			...
			LogDebug("Sign rsa->n (length=%d) with signature (length=%d, truelength=%d message_len=%d) ret = %d ERROR?=%s",
				RSA_size(rsa),
...
//	TODO fix rsa modulus bi_set_as_hex( modulus_N0, BN_bn2hex( issuer->key_chain[0]->n));
    BIGNUM *bn_modulus = (BIGNUM *) RSA_get0_n(issuer->key_chain[0]);
...
//    char *argvtest = "test_join"; // fix for java integration test
	issuer = initIssuer( DEFAULT_KEY_CHAIN_LENGTH, filename, argv[0], hContext);
//    issuer = initIssuer( DEFAULT_KEY_CHAIN_LENGTH, filename, argvtest, hContext);
...
//    credential_filename = "credential_";
//    strcat(credential_filename, bf);
    ...
    fprintf(file, "%s\n", dump_byte_array( credentialRequest.capitalNiLength,
