...
//#include "../../../include/spi_internal_types.h"
//#include "spi_utils.h"
#include <trousers/trousers.h>
#include <spi_utils.h>

//#include "../../../include/tcs_utils.h"
#include <obj.h>
#include <tcs_utils.h>
#include "tsplog.h"
#include "tss/tcs.h"
...
//    result |= Trspi_Hash_UINT32(&hashCtx,  inputData0);
//    result |= Trspi_Hash_UINT32(&hashCtx, inputSize1);
//    result |= Trspi_Hash_UINT32(&hashCtx, inputData1);
...
//////	Trspi_LoadBlob_UINT32((UINT64 *) &offset, TPM_ORD_DAA_Join, hashblob);
...
//	LogDebug("TCSP_DAAJoin(tcsContext=%x,hTPM=%x,STAGE=%x,inputSize0=%x,inputData0=%x,inputSize1=%x,inputData1=%x,ownerAuth=%x)\n",
//		tcsContext,
//		hTPM,
//		stage,
//		inputSize0,
...
//        bi_set_as_BIGNUM(issuer_authentication_PK_i, (BIGNUM *) issuer_authentication_PK[i]);
...
//
//// TODO figure out how to fix missing functions
//    if( (result = obj_tpm_is_connected(  hTPM, &tspContext)) != TSS_SUCCESS)
//        return result;
//
////	if( (result = obj_daaissuerkey_get_daa_handle(hIssuerKey, &daaHandle)))
//    if( (result = obj_daaissuerkey_get_handle_tpm(hIssuerKey, &daaHandle)))
//		return result;
//
////    obj_daa_set_handle_tpm( hDAA, hTPM);
//
//	stage = 0;
//    UINT32 inputSize0 = (UINT32) sizeof(UINT32);
//    UINT32 inputData0 = issuerAuthPKsLength;
//    UINT32 inputSize1 = (UINT32) sizeof(UINT32);
//    UINT32 inputData1 = 0;
//
//	result = Trspi_HashInit(&hashCtx, TSS_HASH_SHA1);
//	result |= Trspi_Hash_UINT32(&hashCtx, TPM_ORD_DAA_Join);
...
//
//	// set the sessionHandle to the returned value
//    UINT64 offset; // TODO fix offset missing definition
//    Trspi_UnloadBlob_UINT32(&offset, &daaHandle, outputData);
//	free(outputData);
////
//// TODO fix the missing function
...
//
//	modulus_length = DAA_PARAM_SIZE_RSA_MODULUS / 8;
//	if ((modulus = malloc(modulus_length)) == NULL) {
//		LogError("malloc of %d bytes failed", modulus_length);
//		result = TSPERR(TSS_E_OUTOFMEMORY);
//		goto close;
//	}
...
//	issuer_settings_bytes = issuer_2_byte_array(issuer_settings, &length);
//#else
//    // TODO fix the missing functions
...
//	}
//#endif
//// TODO fix missing definitions for issuer and issuer_length
//
...
//        if ((result = TCS_API(tspContext)->GetTPMCapability(tspContext, TPM_CAP_VERSION,
//								 0, NULL, &buf_len, &buf)))
//		goto close;
//
//	offset = 0;
//	Trspi_UnloadBlob(&offset, buf_len, buf, &daaIdentityProof.versionInfo);
//	free_tspi(tspContext, buf);
//
//// TODO fix #error tag
...
////		goto close;
////    // TODO fix missing declarations for capitalUprime and capitalUprimeLength
////    BYTE **capitalUPrime;
...
////
//close:
////// TODO fix missing declaration of issuer authentication pk i
////	FREE_BI( issuer_authentication_PK_i);
//	LogDebug("result = %d", result);
//	LogDebug("outputSize=%d", outputSize);
//	LogDebug("outputData=%s", dump_byte_array( outputSize, outputData));
...
get_public_EK(TSS_HTPM hTPM,
	...
	UINT32 uiAttrSize;
	BYTE *pAttr;
	if( (result = obj_tpm_get_policy( hTPM, TSS_POLICY_USAGE, &hTpmPolicy)) != TSS_SUCCESS) {
...
//	struct tdTSS_DAA_PSEUDONYM *signature_pseudonym;
