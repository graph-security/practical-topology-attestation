...
//	LogDebug("encoded issuer_pk[%d]:%s", length, dump_byte_array( length, buffer));
	...
	EVP_MD_CTX_destroy(mdctx);
	LogDebug("\n compute_sign_challenge_host[%d]:%s",
		*result_length,
		dump_byte_array( *result_length, buffer));
	return buffer;
}

static inline int is_element_gamma( bi_ptr capital_nv, TSS_DAA_PK_internal *issuer_pk) {
...
(	TSS_HDAA hDAA,	// in
	...
	*isCorrect = FALSE;
	...
	issuer_pk = e_2_i_TSS_DAA_PK( &hPubKeyIssuer);
	if( issuer_pk == NULL) {
...
//	}
    ...
    LogDebug("capitalZ: %s", bi_2_hex_char(capital_z));
    LogDebug("signature->capitalT: %s", bi_2_hex_char(signature->capitalT));
    ...
    LogDebug("issuer_pk->capitalR0: %s", bi_2_hex_char(issuer_pk->capitalR0));
    LogDebug("signature->sF0: %s", bi_2_hex_char(signature->sF0));
	//  capital_THat = ( capital_THat * tmp1 ) % n
	bi_mul( capital_THat, capital_THat, tmp1);
	bi_mod( capital_THat, capital_THat, n);
    ...
    LogDebug("issuer_pk->capitalR1: %s", bi_2_hex_char(issuer_pk->capitalR1));
    LogDebug("signature->sF1: %s", bi_2_hex_char(signature->sF1));
    ...
    LogDebug("issuer_pk->capitalS: %s", bi_2_hex_char(issuer_pk->capitalS));
    LogDebug("signature->sV: %s", bi_2_hex_char(signature->sV));
	//  capital_THat = ( capital_THat * tmp1 ) % n
	bi_mul( capital_THat, capital_THat, tmp1);
	bi_mod( capital_THat, capital_THat, n);
...
//	}
    ...
    bi_mul( capital_THat, capital_THat, product_r);
	bi_mod( capital_THat, capital_THat, n);
    LogDebug("capital_THat : %s", bi_2_hex_char( capital_THat));
...
and issuer_pk are not consistent (%d)\n",
	...
	if( verifier_transaction == NULL ||
        ...
        TSS_DAA_PSEUDONYM sps = signature_ext.signedPseudonym;

		// anonymity revocation not enabled
//		pseudonym_plain = (TSS_DAA_PSEUDONYM_PLAIN_internal *) sps; //ignature_ext.signedPseudonym;
//		capital_nv = bi_set_as_nbin( pseudonym_plain->capitalNvLength,
//							pseudonym_plain->capitalNv);

// TODO fix the computation for outputing the N_V pseudonym
//pseudonym_plain = sps
        capital_nv = bi_set_as_nbin( sps.payloadLength,                            sps.payload);
//TODO
	...
	ch = compute_sign_challenge_host(
		...
		NULL,//commitment_proofs,
		capital_nv,
		capital_ntilde_v,
		NULL, // verifier_transaction->anonymity_revocator_pk,
		NULL,//pseudonym_enc,
		NULL//pseudonym_encryption_proof
        );
